      ...
      <mu-container fluid style="padding-top: 20px;height: 100%;overflow: auto">
        <mu-row>
          <mu-col sm="12" md="12" >
            ...
            <b>发起人：</b>{{this.$store.state.regularOld.user}}
          </mu-col>
        </mu-row>
        <br>
        <mu-row>
          <mu-col sm="12" md="12">
            <mu-divider></mu-divider>
            <b>参与人员：</b><span v-for="user in this.$store.state.regularOld.users" :key="user">{{user}}</span>
          </mu-col>
        </mu-row>
        <br>
        <br>
        <mu-row>
          <mu-col sm="12">
            <mu-divider></mu-divider>
            <b>活动时间：</b>{{this.$store.state.regularOld.begdate}} &#45;&#45; {{this.$store.state.regularOld.enddate}}
          </mu-col>
        </mu-row>
        <br>
        <mu-row>
          <mu-col sm="12" md="12">
            ...
            <mu-flex warp="warp" direction="column">
              <mu-flex :key="book" v-for="book in this.$store.state.regularOld.bookList" warp="warp">
                {{book}}
              </mu-flex>
            </mu-flex>
          </mu-col>
        ...
        <mu-row>
          <mu-col sm="12">
            <b>备选图书：</b>
            <mu-flex warp="warp" direction="column">
              <mu-flex :key="book" v-for="book in this.$store.state.regularOld.rebookList" warp="warp">
                {{book}}
              </mu-flex>
            </mu-flex>
          </mu-col>
        ...
        <mu-row v-for="(info, index) in this.$store.state.regularOld.userinfo" :key="info.id">
          <mu-col sm="12">
...
<script>
import {GetOldData, GetOldTotal} from "../../../api/RegularApi";
...
export default {
  ...
  methods: {
    ...
    getOldData (page) {
      ...
      GetOldData(this.current).then((response) => {
        this.$store.commit('REGULAR_OLD_UPDATE', response.data)
        if (response.status === 200) {
          loading.close()
        }
      })
